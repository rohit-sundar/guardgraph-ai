...
function safeSend(kind, value, extra) {
  try {
    send(Object.assign({ kind, value, t: Date.now() - _t0 }, extra || {}));
  } catch (e) {
  }
}
...
var OVERLAY_ATTACK_WINDOW_TYPES = /* @__PURE__ */ new Set([
  2002,
  // TYPE_PHONE — legacy pre-O overlay
  2003,
  // TYPE_SYSTEM_ALERT — legacy pre-O overlay, the classic phishing-overlay type
  2006,
  // TYPE_SYSTEM_OVERLAY — legacy, draws over other apps without taking focus
  2007,
  // TYPE_PRIORITY_PHONE — legacy
  2032,
  // TYPE_ACCESSIBILITY_OVERLAY — the modern path banking trojans actually use, drawn by an app's own bound AccessibilityService
  2038
  // TYPE_APPLICATION_OVERLAY — the modern (API 26+) SYSTEM_ALERT_WINDOW type
]);
...
function isNoiseFilePath(path) {
  return path.indexOf("/data/misc/profiles/") === 0 || path.indexOf("profileinstaller_profileWrittenFor") !== -1 || path.endsWith(".prof") || path.endsWith(".profm");
}
...
frida_java_bridge_default.perform(function() {
  hookNetwork(frida_java_bridge_default);
  hookSms(frida_java_bridge_default);
  hookSmsRead(frida_java_bridge_default);
  hookDcl(frida_java_bridge_default);
  hookNativeLibraryLoad(frida_java_bridge_default);
  hookAccessibility(frida_java_bridge_default);
  hookCrypto(frida_java_bridge_default);
  hookOverlay(frida_java_bridge_default);
  hookContentResolverQuery(frida_java_bridge_default);
  hookClipboard(frida_java_bridge_default);
  hookUrls(frida_java_bridge_default);
  hookFileWrites(frida_java_bridge_default);
  hookProcessExec(frida_java_bridge_default);
  safeSend("ready", true);
});
